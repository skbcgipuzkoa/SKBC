...
export default async function KenshiDetailPage({
  ...
  const [{ data: attendance }, { data: exams }, { data: courses }, childRankingResult, childNotesResult, childNoticesResult, childBehaviorResult, childTransitionResult, blackBeltResult, shakujoResult] = await Promise.all([
    ...
    supabase
      .from("exams")
      .select("exam_date,grade,examiner,diploma_url,report_url")
      .eq("member_id", member.id)
        ...
        <section className="profile-grid">
          ...
          <article className="card detail-list">
            ...
            <div className="profile-actions">
              {member.ficha_token ? <a className="text-link" href={`/ficha/${member.ficha_token}?admin=1&returnTo=${encodeURIComponent(`/kenshis/${member.legacy_id ?? ""}`)}`} target="_blank" rel="noopener noreferrer external">Abrir ficha nueva</a> : (
                ...
                <tbody>
                  {(exams ?? []).map((item) => (
                    <tr key={`${item.exam_date}-${item.grade}`}>
                      ...
                      <td data-label="Informe">{item.report_url ? <a className="text-link" href={item.report_url} target="_blank">Abrir informe</a> : "-"}</td>
                      <td data-label="Diploma">{item.diploma_url ? <a className="text-link" href={item.diploma_url} target="_blank">Abrir diploma</a> : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
